...
public class FichajeRestController {
    ...
    public ResponseEntity<?> show(@PathVariable Long id) {
        ...
        try {
            List<Fichaje> fichajes = fichajeService.findByEmpleado(id);
            System.out.println(fichajes.get(0).getEmpleado().getEmail());
            if (fichajes == null) {
                response.put("mensaje", "No existen fichajes para el ID: " + id);
