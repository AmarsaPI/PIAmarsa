...
object FichajeApiService {
    ...
    val formatterDateTime = DateTimeFormatter.ISO_LOCAL_DATE_TIME
    val formatterDate = DateTimeFormatter.ISO_LOCAL_DATE
    val gsonConverter = GsonBuilder()
        // ----- CONFIGURACIÓN PARA LOCALDATETIME (Fichajes, createdAt...) -----
        .registerTypeAdapter(LocalDateTime::class.java, JsonSerializer<LocalDateTime> { src, _, _ ->
            JsonPrimitive(src.format(formatterDateTime))
        })
        .registerTypeAdapter(LocalDateTime::class.java, JsonDeserializer { json, _, _ ->
            LocalDateTime.parse(json.asString, formatterDateTime)
        })

        // ----- CONFIGURACIÓN PARA LOCALDATE (Festivos...) -----
        .registerTypeAdapter(LocalDate::class.java, JsonSerializer<LocalDate> { src, _, _ ->
            com.google.gson.JsonPrimitive(src.format(formatterDate))
        })
        .registerTypeAdapter(LocalDate::class.java, JsonDeserializer { json, _, _ ->
            LocalDate.parse(json.asString, formatterDate)
        })
        .create()
    ...
    val apiService: ApiService by lazy {
        Retrofit
            ...
            .addConverterFactory(GsonConverterFactory.create(gsonConverter)) // Usamos el Gson configurado
            .build()
