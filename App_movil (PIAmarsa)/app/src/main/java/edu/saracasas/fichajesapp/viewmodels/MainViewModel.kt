...
class MainViewModel(app: Application): AndroidViewModel(app) {
    ...
    suspend fun getHorariosByEmpleadoId() {
        _horariosEmpleado.update {
            repository.getHorariosByEmpleadoId(_empleadoLogueado.value!!.id,
                "2026-05-01", "2026-05-31")?: emptyList()
        }
    }
    ...
    fun loginEmpleado(email: String, password: String) {
        viewModelScope.launch {
            ...
            if (loginResult?.empleado != null) {
                ...
                async { getFichajesByEmpleadoIdSemanaActual() }.await()
                async { getHorariosByEmpleadoId() }.await()
            } else {
