...
import edu.saracasas.fichajesapp.data.models.Ausencia
...
import edu.saracasas.fichajesapp.ui.screens.ObtenerSemanaActual
...
class MainViewModel(app: Application): AndroidViewModel(app) {
    ...
    private val _startWeek = MutableStateFlow(
        ObtenerSemanaActual(LocalDate.now()))
    val startWeek: StateFlow<LocalDate> = _startWeek

    private val _endWeek = MutableStateFlow(
        ObtenerSemanaActual(LocalDate.now(), false))
    val endWeek: StateFlow<LocalDate> = _endWeek
    ...
    private val _horariosEmpleado = MutableStateFlow<List<HorarioResponse>?>(emptyList())
    val horariosEmpleado: StateFlow<List<HorarioResponse>?> = _horariosEmpleado

    private val _currentMonth = MutableStateFlow<YearMonth>(YearMonth.now())
    val currentMonth: StateFlow<YearMonth> = _currentMonth

    private val _ausenciasEmpleado = MutableStateFlow<List<Ausencia>?>(null)
    val ausenciasEmpleado: StateFlow<List<Ausencia>?> = _ausenciasEmpleado
    ...
    suspend fun getFichajesByEmpleadoIdSemanaActual() {
        val rango = RangoFechasDTO(_startWeek.value.toString(), _endWeek.value.toString())
        _fichajesSemanaActual.update {
            repository.getFichajesByEmpleadoIdSemanaActual(_empleadoLogueado.value!!.id,
                rango) ?: emptyList() }
    }

    suspend fun getVacacionesByEmpleadoId() {
        _ausenciasEmpleado.update {
            repository.getVacacionesByEmpleadoId(_empleadoLogueado.value!!.id) ?: emptyList()
        }
    }

    /** Función para obtener los horarios de un empleado por su ID, actualiza el StateFlow de horarios del empleado con los datos obtenidos del Repository.
     * Se ejecuta en una corrutina dentro del viewModelScope para no bloquear el hilo principal.
     */
    fun semanaActualView(sumar: Boolean = true) {
        if (sumar) {
            _startWeek.update { it.plusWeeks(1) }
            _endWeek.update { it.plusWeeks(1) }
        } else {
            _startWeek.update { it.minusWeeks(1) }
            _endWeek.update { it.minusWeeks(1) }
        }
    ...
    fun mesActualView(sumar: Boolean = true) {
        if (sumar) {
    ...
    suspend fun getHorariosByEmpleadoId() {
        val firstDayOfMonth =_currentMonth.value.atDay(1).toString()
        val lastDayOfMonth = _currentMonth.value.atEndOfMonth().toString()
        _horariosEmpleado.update {
            val horariosMes = repository.getHorariosByEmpleadoId(_empleadoLogueado.value!!.id,
     ...
     * Se ejecuta en una corrutina dentro del viewModelScope para no bloquear el hilo principal.
     */
    suspend fun loginEmpleado(email: String, password: String) {
        val loginResult = repository.loginEmpleado(email, password)
