...
class MainViewModel(app: Application): AndroidViewModel(app) {
    ...
    fun getFichajesByEmpleadoId(empleado_id: Long) {
        viewModelScope.launch {
            _allFichajes.update { async { repository.getFichajesByEmpleadoId(empleado_id) }.await() ?: emptyList() }
        }
    }

    fun loginEmpleado(email: String, password: String) {
        viewModelScope.launch {
            _empleadoLogueado.value = async { repository.loginEmpleado(email, password) }.await()
        }
    }
}